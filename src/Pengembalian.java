...
public class Pengembalian extends javax.swing.JFrame {
    ...
    public Pengembalian() {
        initComponents();
        Connection_sql DB = new Connection_sql();
        update.setEnabled(false);
        delete.setEnabled(false);
        clear.setEnabled(false);
        conn = DB.get_con();
        try {
            state_p = conn.createStatement();
    ...
    private void hitung_lama(){
        try {
            ...
            rs=state_p.executeQuery("SELECT `harga_rental` FROM `tb_sepeda` WHERE id_sepeda LIKE '%"+sepeda.getText()+"%'");
                if(rs.next()){
    ...
    private void saveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveActionPerformed
        try{
            ...
            String I_sql = "INSERT INTO `tb_pengembalian`(`id_pengembalian`, `id_rental`, `id_penyewa`, `id_sepeda`, `tgl_rental`, `tgl_kembali`, `lama`, `total_bayar`, `Status`) VALUES('"+kode_kembali.getText()+"','"+rental.getText()+"','"+perental.getText()+"','"+sepeda.getText()+"','"+tanggal_rental+"','"+tanggal_kembali_rental+"','"+lamaa+"','"+harga.getText()+"','"+status.getSelectedItem()+"')";
            state_p.executeUpdate(I_sql);
    ...
    private void sepedaKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_sepedaKeyReleased
        ...
        if (banyak==10){
            try{
                ...
                rs=state_p.executeQuery("SELECT `id_sepeda`, `nama_sepeda`, `merk_sepeda`, `bm_sepeda`, `harga_rental`, `Status` FROM `tb_sepeda` WHERE id_sepeda LIKE '%"+sepeda.getText()+"%'");
                if(rs.next()){
    ...
    private void perentalKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_perentalKeyReleased
        ...
        if (banyak==10){
            try{
                ...
                update.setEnabled(true);
                delete.setEnabled(true);
                rs=state_p.executeQuery("SELECT `id_penyewa`, `nama_penyewa`, `alamat`, `no_hp`, `no_kto`, `Status` FROM `tb_penyewa` WHERE `id_penyewa` LIKE '%"+perental.getText()+"%'");
                if(rs.next()){
    ...
    private void rentalKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_rentalKeyReleased
        ...
        if (banyak==10){
            try{
                ...
                update.setEnabled(true);
                delete.setEnabled(true);
                rs=state_p.executeQuery("SELECT `id_rental`, `id_penyewa`, `id_sepeda`, `nama_penyewa`, `nama_sepeda`, `harga_rental`, `tgl_rental`, `Status` FROM tb_rental WHERE `id_rental` LIKE '%"+rental.getText()+"%'");
                if(rs.next()){
    ...
    private void updateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateActionPerformed
        try {
            String U_sql = "UPDATE `tb_pengembalian` SET `id_rental`='"+rental.getText()+"',`id_penyewa`='"+perental.getText()+"',`id_sepeda`='"+sepeda.getText()+"',`tgl_rental`='"+tanggal_rental+"',`tgl_kembali`='"+tanggal_kembali_rental+"',`lama`='"+lama_rental.getText()+"',`total_bayar`='"+harga.getText()+"',`Status`='"+status.getSelectedItem()+"' WHERE id_pengembalian = '"+kode_kembali.getText()+"'";
            state_p.executeUpdate(U_sql);
    ...
    private void deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteActionPerformed
        try {
            String D_sql = "DELETE FROM `tb_pengembalian` WHERE id_pengembalian = '"+kode_kembali.getText()+"'";
            state_p.executeUpdate(D_sql);
